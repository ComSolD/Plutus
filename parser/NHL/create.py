...
def сreate():
    ...
    cur.execute(f"CREATE TABLE IF NOT EXISTS `player_stat`(stat_ID VARCHAR(36) PRIMARY KEY, player_ID VARCHAR(36), match_ID VARCHAR(36), team_ID VARCHAR(36), position VARCHAR(20), G INTEGER, A INTEGER, plusMinus INTEGER, S INTEGER, SM INTEGER, BS INTEGER, PN INTEGER, PIM INTEGER, HT INTEGER, TK INTEGER, GV INTEGER, SA INTEGER, GA INTEGER, SV INTEGER, SVProcent INTEGER, ESSV INTEGER, PPSV INTEGER, FOREIGN KEY (player_ID) REFERENCES player (player_ID), FOREIGN KEY (match_ID) REFERENCES match (match_ID), FOREIGN KEY (team_ID) REFERENCES team (team_ID));")
    conn.commit()
    ...
    cur.execute(f"CREATE TABLE IF NOT EXISTS `team_pts_stat`(team_pts_stat_ID VARCHAR(36) PRIMARY KEY, match_ID VARCHAR(36), team_ID VARCHAR(36), total INTEGER, totalMissed INTEGER, total_T1 INTEGER, total_T1Missed INTEGER, total_T2 INTEGER, total_T2Missed INTEGER, total_T3 INTEGER, total_T3Missed INTEGER, FOREIGN KEY (match_ID) REFERENCES match (match_ID), FOREIGN KEY (team_ID) REFERENCES team (team_ID));")
    conn.commit()
