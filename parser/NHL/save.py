...
def bet_old_resul_tables(match_ID, teams, resul_team1, match_total, bet):
    ...
    if len(inf) != 0:
        ...
        cur.execute(f'''INSERT INTO `bet`(bet_ID, match_ID, team1_ID, team2_ID, {"ML_team1_parlay" if bet[0] == 'Team1' else "ML_team2_parlay"}, ML_resul, total, total_resul) VALUES('{bet_ID}', '{match_ID}', '{teams[0]}', '{teams[1]}', {bet[1]}, '{teams[0] if resul_team1 == "Win" else teams[1]}', {bet[2]}, '{'over' if bet[2] < (int(match_total[0]) + int(match_total[1])) else 'under'}');''')
        conn.commit()
...
def match_table(match_ID, teams, status, season, stage, date_match):
    ...
    cur.execute(f"INSERT INTO `match`(match_ID, team1_ID, team2_ID, status, season, stage, date) VALUES('{match_ID}', '{teams[0]}', '{teams[1]}', '{status}', '{season}', '{stage}', '{date_match}')")
    conn.commit()
...
def team_stat_pts_tables(match_ID, teams_ID, total):
    ...
    cur.execute(f"INSERT INTO `team_pts_stat`(team_pts_stat_ID, match_ID, team_ID, total, totalMissed, total_T1, total_T1Missed, total_T2, total_T2Missed, total_T3, total_T3Missed) VALUES('{team1_PTS_Stat_ID}', '{match_ID}', '{teams_ID[0]}', {total[0][-2]}, {total[0][-1]}, {total[0][0]}, {total[0][1]}, {total[0][2]}, {total[0][3]}, {total[0][4]}, {total[0][5]})")
    conn.commit()

    team2_PTS_Stat_ID = str(uuid.uuid4())
    cur.execute(f"INSERT INTO `team_pts_stat`(team_pts_stat_ID, match_ID, team_ID, total, totalMissed, total_T1, total_T1Missed, total_T2, total_T2Missed, total_T3, total_T3Missed) VALUES('{team2_PTS_Stat_ID}', '{match_ID}', '{teams_ID[1]}', {total[1][-2]}, {total[1][-1]}, {total[1][0]}, {total[1][1]}, {total[1][2]}, {total[1][3]}, {total[1][4]}, {total[1][5]})")
    conn.commit()
...
def player_tables(match_ID, team_ID, forwards_team, defensemen_team, goalies_team):
    ...
    for player_name in forwards_team:
        ...
        cur.execute(f'''INSERT INTO `player_stat`(stat_ID, player_ID, match_ID , team_ID, position, G, A, plusMinus, S, SM, BS, PN, PIM, HT, TK, GV) VALUES("{stat_ID}", "{player_ID}", "{match_ID}", "{team_ID}", "Forward", {player_name[0]}, {player_name[1]}, {player_name[2]}, {player_name[3]}, {player_name[4]}, {player_name[5]}, {player_name[6]}, {player_name[7]}, {player_name[8]}, {player_name[9]}, {player_name[10]})''')
        conn.commit()
    ...
    for player_name in defensemen_team:
        ...
        cur.execute(f'''INSERT INTO `player_stat`(stat_ID, player_ID, match_ID, team_ID, position, G, A, plusMinus, S, SM, BS, PN, PIM, HT, TK, GV) VALUES("{stat_ID}", "{player_ID}", "{match_ID}", "{team_ID}", "Defense", {player_name[0]}, {player_name[1]}, {player_name[2]}, {player_name[3]}, {player_name[4]}, {player_name[5]}, {player_name[6]}, {player_name[7]}, {player_name[8]}, {player_name[9]}, {player_name[10]})''')
        conn.commit()
    ...
    for player_name in goalies_team:
        ...
        cur.execute(f'''INSERT INTO `player_stat`(stat_ID, player_ID, match_ID, team_ID, position, SA, GA, SV, SVProcent, ESSV, PPSV, PIM) VALUES("{stat_ID}", "{player_ID}", "{match_ID}", "{team_ID}", "Goaltender", {player_name[0]}, {player_name[1]}, {player_name[2]}, {player_name[3]}, {player_name[4]}, {player_name[5]}, {player_name[6]})''')
        conn.commit()
